...
function Build-CISAz1250($findings)
{
	...
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISAz1250"
		FindingName	     = "CIS Az 1.25 - There are less than 1 or more than 5 users with an global administrator assignment"
...
function Audit-CISAz1250
{
	Try
	{
		...
		{
			$GlobalAdminList | Format-Table -AutoSize | Out-File "$path\CISAz1250-GlobalAdmins.txt"
			$endobject = Build-CISAz1250($globalAdmins.AdditionalProperties.Count)
			Return $endobject
		}
